...
main <- function() {
    validation_end <- 2017
    validation_end <- as.numeric(paste0(validation_end, "000000"))
    train_end <- 2018
    train_end <- as.numeric(paste0(train_end, "000000"))

    all_data <- process_data()
    train <- all_data[all_data$DATE < validation_end, ]
    validation <- all_data[all_data$DATE >= validation_end & all_data$DATE < train_end, ]
    ...
    validation$CAT <- ifelse(validation$VMAX <= 33, "TD",
        ifelse(validation$VMAX <= 63, "TS",
            ifelse(validation$VMAX <= 95, "Min", "Maj")))
    ...
    print(paste0("Validation data size: ", nrow(validation)))
    ...
    write.csv(validation, "HIECNN/IMERG/DEV/ALL_VALIDATION_DATA.csv", row.names = FALSE)
